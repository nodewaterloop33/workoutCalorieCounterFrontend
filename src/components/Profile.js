...
const Profile = () => {
  ...
  const onSubmit = (data) => {
    ...
    fetch(`${process.env.REACT_APP_API_URLser.id}`, {
      method: "PATCH",
      headers: {
...
const getRecordsByUserId = async (id) => {
    let resp = await fetch(
      `${process.env.REACT_APP_API_URL}records?userId=${id}`
    );
    return resp.json();
  };
...
const ReocrdByDate = () => {
  ...
  useEffect(() => {
    // console.log(date);
    fetch(`${process.env.REACT_APP_API_URL}records?userId=${currentUser.id}&date=${date}`)
        .then((resp) => resp.json())
        .then((data) => {
          setRecords(data);
        });
